...
import { getCachedIdentification, setCachedIdentification } from "./cache";
import { buildEmbeddingPrompt, buildIdentificationPrompt } from "./prompts";
import { geminiRateLimiter, GeminiRateLimiter } from "./rate-limiter";
import type {
  GeminiCondition,
...
function getExpoExtra(): Record<string, unknown> | undefined {
  return (Constants.expoConfig?.extra ?? undefined) as Record<string, unknown> | undefined;
}

export function getGeminiApiKey(): string {
  const apiKey = getExpoExtra()?.geminiApiKey;

  if (typeof apiKey !== "string" || apiKey.trim().length === 0) {
    throw new Error(
      "Missing Gemini API key. Set extra.geminiApiKey in your Expo config before starting the app.",
    );
  }
...
function normalizeConditionRange(value: unknown, fallback: GeminiCondition): [GeminiCondition, GeminiCondition] {
...
function logUsage(operation: string, usage?: GeminiUsageMetadata): void {
  if (!usage) {
    console.info(`[Gemini] ${operation} completed. Estimated cost: $${ESTIMATED_COST_PER_REQUEST_USD.toFixed(3)}`);
    return;
  }
...
export class GeminiClient {
  ...
  private async postJson<T>(
    ...
    try {
      ...
      if (!response.ok) {
        const message = responseText || `Gemini request failed with status ${response.status}.`;
