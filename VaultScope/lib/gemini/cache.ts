...
import type { GeminiCacheEntry, GeminiIdentifyResponse } from "./types";
...
function fingerprintImage(base64Image: string): string {
  ...
  // This is an approximation of a perceptual-style cache key without native image decoding.
...
export async function clearCachedIdentification(
  images: string[],
  category: string,
): Promise<void> {
  await AsyncStorage.removeItem(buildCacheKey(images, category));
}
